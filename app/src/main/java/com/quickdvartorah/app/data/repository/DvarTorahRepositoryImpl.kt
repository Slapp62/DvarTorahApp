...
class DvarTorahRepositoryImpl @Inject constructor(
    ...
    override fun getLikedDvareiTorah(uid: String): Flow<List<DvarTorah>> = callbackFlow {
        val listener = firestore.collectionGroup(FirestoreConstants.COLLECTION_LIKES)
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                val dvarRefs = snapshot?.documents
                ...
                Tasks.whenAllSuccess<DocumentSnapshot>(dvarRefs.map { it.get() })
                    .addOnSuccessListener { docs ->
                        val items = docs
                            .mapNotNull { it.toObject(DvarTorah::class.java) }
                            .filter { it.status == FirestoreConstants.DvarTorahStatus.PUBLISHED }
                            .sortedByDescending { it.createdAt?.seconds ?: 0L }
                        trySend(items)
                    }
                    .addOnFailureListener { taskError ->
                        close(kotlinx.coroutines.CancellationException(taskError.message ?: "Could not load saved items", taskError))
                    }
            }
        awaitClose { listener.remove() }
    }
